...
export function UlCustomProducts({ customProducts }) {
  ...
  const productosPorCategoria = customProducts.reduce((acc, prod) => {
  ...
  const toggleExpandedAll = () => {
    if (expandedAll) {
      // Si estaban todos expandidos → colapsa todos
      setExpandedAll(false);
      setExpandedItems({});
    } else {
      // Expande todos
      const all = {};
      customProducts.forEach((p) => (all[p.id] = true));
  ...
  const toggleOne = (id) => {
    // Si están todos expandidos, el click no hace nada
